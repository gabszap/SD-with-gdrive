...
def main():
    ...
    if '-y' in sys.argv:
        ...
        while True:
            choice = input("Does it decrypt all files? (y/n)：")
            if choice.lower() == 'y':
                break
            elif choice.lower() == 'n':
                return
    ...
    for filename in os.listdir(decrypt_dir):
        if filename.endswith('.jpg') or filename.endswith('.png') or filename.endswith('.webp') or filename.endswith('.jpeg'): 
            ...
            if os.path.exists(output_filename):
                ...
                print(f"The {filename} file has been skipped.")
                continue
            futures.append(executor.submit(process_image, filename, output_filename, password))
                    
    if skipped_files:
        file_count - len(skipped_files)
        print("The following documents have been skipped：")
        for filename in skipped_files:
            print(filename)
print('Translated by Gabs, not my code. author: https://github.com/viyiviyi/sd-encrypt-image')            
